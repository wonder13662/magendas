...
toast_master.mobile_list_manager = {
	...
	*/
	...
	/*
		@ Public
		@ Desc : 고정된 갯수의 타이머 테이블을 그립니다. ex) speaker, evaluator
	*/	
	,add_member_timer_table_fixed:function(title, append_target_jq, event_toggle_controller, meta_data) {

		if(_v.isNotValidStr(title)){
			console.log("!Error! / mobile.list.manager.js / add_member_timer_table_fixed / _v.isNotValidStr(title)");
			return;
		}

		if(append_target_jq ==  undefined){
			console.log("!Error! / mobile.list.manager.js / add_member_timer_table_fixed / append_target_jq ==  undefined");
			return;
		}

		if(event_toggle_controller ==  undefined){
			console.log("!Error! / mobile.list.manager.js / add_member_timer_table_fixed / event_toggle_controller ==  undefined");
			return;
		}
	...
	*/	
	,add_timer_table_fixed:function(title, append_target_jq, event_toggle_controller, delegate_on_time_update, meta_data) {
		...
		var add_timer = function(time_arr, row_jq, event_toggle_controller, delegate_data_param, meta_data) {
			...
			timer_controller.hide_delete_btn();
		...
		// 고정 열에서는 버튼 이벤트를 사용하지 않습니다.
		add_timer_btn_controller.disable();
		add_timer_btn_controller.off();
